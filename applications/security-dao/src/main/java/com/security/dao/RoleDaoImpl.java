...
//		qry.setParameter(0, userName);
		List<Role> roles = (List<Role>) getHibernateTemplate().findByNamedParam("SELECT r FROM Role r join r.users u WHERE u.userName = :userName","userName", userName);
...
//		}
	...
	public List<Role> findRolesByUserId(Long id) {
		List<Role> roles = (List<Role>) getHibernateTemplate().findByNamedParam("SELECT r FROM Role r join r.users u WHERE u.id = :id","id", id);
