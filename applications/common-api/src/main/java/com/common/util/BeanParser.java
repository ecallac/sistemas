...
public class BeanParser {
	 ...
	 * @param objectTo
	 * @param customMapping
	 *            is Map<String, String> e.g. map.put("fieldA", "fieldB"); this is
	 *            objectFrom.fieldA --> objectTo.fieldB
	 * @return
	 */
						...
						if (value != null) {
							try {
								for (Class<?> toClazz : classesTo) {
									try {
										Field toField = toClazz
												.getDeclaredField(customMapping.get(fromField.getName()));
	 ...
	 * @param customMapping
	 *            is Map<String, String> e.g. map.put("fieldA", "fieldB"); this is
	 *            objectFrom.fieldA --> toClass.fieldB
	 * @return
	 */
	...
	public static Object parseObjectToNewClass(Object objectFrom, Class<?> toClass) {
		return parseObjectToNewClass(objectFrom, toClass, null);
	}

	public static List<Object> convertList(List<?> srcList, Class<?> toClass) {
		List<Object> targetBeans = new ArrayList<Object>();
		for (Object src : srcList) {
			targetBeans.add(convert(src, toClass));
		}
		return targetBeans;
	}

	public static Object convert(Object src, Class<?> toClass) {
		try {
			Object toClassInstanced = toClass.newInstance();
			BeanUtils.copyProperties(src, toClassInstanced);
			return toClassInstanced;
		} catch (Exception e) {
